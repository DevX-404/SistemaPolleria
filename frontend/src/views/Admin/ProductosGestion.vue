          ...
          <table v-else class="table align-items-center mb-0">
            <thead>
              <tr>
                ...
                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Categoría
                </th>
              ...
              <tr v-for="producto in productos" :key="producto.id_producto">
                <td>
                  <div class="d-flex px-2 py-1">
                    <div>
                      <img :src="getImgPath(producto.imagen_path)" class="avatar avatar-sm me-3 rounded-3 shadow-sm"
                        style="width: 45px; height: 45px; object-fit: cover;" alt="prod">
                    </div>
                    <div class="d-flex flex-column justify-content-center">
                      <h6 class="mb-0 text-sm fw-bold text-dark">{{ producto.nombre }}</h6>
                      <p class="text-xs text-muted mb-0">{{ producto.descripcion ? producto.descripcion.substring(0, 35)
                        + '...' : 'Sin descripción' }}</p>
                    </div>
                  </div>
                </td>
                ...
                <td class="align-middle text-center">
                  <span v-if="producto.stock === null || producto.stock > 10"
                    class="badge bg-light-success">Disponible</span>
                  <span v-else-if="producto.stock > 0" class="badge bg-light-warning">Poco Stock ({{ producto.stock
                    }})</span>
                ...
                <td class="align-middle text-end">
                  ...
                  <button class="btn btn-icon btn-sm btn-light-danger text-danger"
                    @click="eliminarProducto(producto.id_producto)" title="Eliminar">
                    🗑️
                  </button>
                </td>
              </tr>
          ...
          <div class="modal-body">
            <form @submit.prevent="guardarProducto">
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control" v-model="productoForm.nombre" placeholder="Ej: Pollo a la Brasa"
                  required>
              </div>
              ...
              <div class="row">
                ...
                <div class="col-md-6 mb-3">
                  ...
                  <div class="input-group">
                    ...
                    <select v-if="!nuevaCategoriaMode" class="form-select" v-model="productoForm.categoria" required>
                      <option value="" disabled>Seleccionar...</option>
                      <option v-for="cat in listaCategorias" :key="cat" :value="cat">{{ cat }}</option>
                    </select>

                    <input v-else type="text" class="form-control" v-model="productoForm.categoria"
                      placeholder="Escribe la nueva..." required>

                    <button class="btn btn-outline-secondary" type="button" @click="toggleNuevaCategoria">
                      <font-awesome-icon :icon="nuevaCategoriaMode ? 'list' : 'plus'" />
                    </button>
                  </div>
                </div>
              </div>
...
const toggleNuevaCategoria = () => {
    nuevaCategoriaMode.value = !nuevaCategoriaMode.value;
    productoForm.value.categoria = ''; 
};
